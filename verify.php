...
$hash = isset($_GET['h']) && is_string($_GET['h']) ? (string)$_GET['h'] : '';

// 🛡️ ЗАХИСТ: Валідуємо формат хеша (SHA-256 = 64 символи hex)
...
$status_text = "Криптографічний підпис не збігається. Можливо, документ був підроблений або ще не підписаний усім керівництвом ППО.";
...
if ($protocol) {
    ...
    $head_clean_name = htmlspecialchars(trim($h_parts[0]) ?: 'Не визначено', ENT_QUOTES, 'UTF-8');
    $sec_clean_name = htmlspecialchars(trim($s_parts[0]) ?: 'Не визначено', ENT_QUOTES, 'UTF-8');

    // Перевіряємо фінальний хеш
    if (!empty($protocol['document_hash']) && hash_equals($protocol['document_hash'], $hash)) {
        ...
        $status_text = "Цей електронний протокол є незмінним. Кваліфіковані електронні підписи підтверджено системою.";
    } 
...
<head>
    ...
    <style>
        ...
        .icon-wrap { width: 80px; height: 80px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 40px; margin: 0 auto 20px; color: <?= $status_color ?>; background: <?= $status_color ?>11; }
...
<div class="verify-card">
    ...
    <h1><?= htmlspecialchars($status_title, ENT_QUOTES, 'UTF-8') ?></h1>
    <p><?= htmlspecialchars($status_text, ENT_QUOTES, 'UTF-8') ?></p>

    <?php if ($is_valid && $protocol): ?>
        <div class="data-list">
            <div class="data-row">
                <span class="data-label">Номер протоколу:</span>
                <span class="data-val">№ <?= htmlspecialchars($protocol['protocol_number'] ?? 'Б/Н', ENT_QUOTES, 'UTF-8') ?></span>
            </div>
            <div class="data-row">
            ...
            <div class="data-row">
                ...
                <span class="data-val"><?= $head_clean_name ?></span>
            </div>
            <div class="data-row">
                <span class="data-label">Секретар:</span>
                <span class="data-val"><?= $sec_clean_name ?></span>
            </div>
        </div>
        ...
        <?php if (!empty($protocol['document_hash'])): ?>
            <div class="data-list" style="background: rgba(59, 130, 246, 0.05); border-color: rgba(59, 130, 246, 0.2);">
                ...
                <div class="data-row" style="flex-direction: column; align-items: flex-start;">
                    ...
                    <span class="data-val hash-text" style="text-align: left; max-width: 100%;"><?= htmlspecialchars($protocol['head_signature_hash'] ?? '', ENT_QUOTES, 'UTF-8') ?></span>
                </div>
                <div class="data-row" style="flex-direction: column; align-items: flex-start;">
                    <span class="data-label" style="margin-bottom: 4px;">Слід Секретаря:</span>
                    <span class="data-val hash-text" style="text-align: left; max-width: 100%;"><?= htmlspecialchars($protocol['manager_signature_hash'] ?? '', ENT_QUOTES, 'UTF-8') ?></span>
                </div>
                <div class="data-row" style="flex-direction: column; align-items: flex-start;">
                    <span class="data-label" style="margin-bottom: 4px; color: #10b981;">Фінальний SHA-256 документа:</span>
                    <span class="data-val hash-text" style="text-align: left; max-width: 100%; color: #10b981;"><?= htmlspecialchars($protocol['document_hash'] ?? '', ENT_QUOTES, 'UTF-8') ?></span>
                </div>
            </div>
        <?php endif; ?>
    <?php endif; ?>
